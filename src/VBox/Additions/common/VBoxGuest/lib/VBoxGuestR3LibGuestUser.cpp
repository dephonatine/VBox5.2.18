                                           ...
                                           uint8_t *puDetails, uint32_t cbDetails)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        AssertPtr(pDynamic);
		/* no checks? can we reach this? */
        memcpy(pDynamic, pszUser, cbUser); // cbUser = controlled. pszUser = controlled. size of pReport->status = 20. pDynamic actually points to pReport->status.szUser. so a 4 byte write will overflow past this object.
        if (cbDomain)
